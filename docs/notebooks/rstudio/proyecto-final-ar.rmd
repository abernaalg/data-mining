...
Escribe la fecha actual (**NOTE-01**): Junio 5 2021
...
```{r}
NOMBRE_COMPLETO = "Andrea Bernal Granados"
DIGITOS_MATRICULA = 01375499
```
...
- Support del item {beer} (**NOTE-06**): 3/5 => 0.6 = 60%
- Support del itemset {beer, nuts} (**NOTE-07**): 1/5 => 0.2 = 20%
...
```{r}
try(
   plot(smalltx, main = paste0("Elaborado por: Andrea Bernal Granados", " (", 01375499, ")" ))
)
...
```{r}
try(
  itemFrequencyPlot(smalltx, topN=10, main = paste0("Elaborado por: Andrea Bernal Granados", " (", 01375499, ")" ))
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Que no hay relación del producto pañales con otro artículo mostrado.
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque la regla support y confidence no incluyen esa regla
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Generar un identificador de todas las sesiones que se han hecho en spotify
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Generar un identificador de todas las canciones o contenido con el que los usuarios han interactuado en spotify
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Si porque a partir de la frecuencia de los parametros que se consideran la plataforma puede pronosticar tendencias y generar asociaciones para  
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Que las reglas de asociacion anteriores no encuentran los  parametros para los valores de support y confidence
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que si le pones valores muy pequeños ya confiabildiad de los resultados ya es muy volatil
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por: Andrea Bernal Granados", " (", 01375499, ")" ))
)
